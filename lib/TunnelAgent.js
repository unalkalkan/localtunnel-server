...
class TunnelAgent extends Agent {
    ...
    listen() {
        ...
        return new Promise((resolve, reject) => {
            getPort(
                {
                    port: this.client_min_port_range,
                    stopPort: this.client_max_port_range,
                },
                (err, port) => {
                    if (err) {
                        reject(err);
                    ...
                    this.server.listen(port, async () => {
                        ...
                        if (!PUBLIC_IP) {
                            try {
                                PUBLIC_IP = await getPublicIPv4();
                            } catch (err) {
                                // Public IP lookup failure should not block tunnel startup.
                            }
                        }
